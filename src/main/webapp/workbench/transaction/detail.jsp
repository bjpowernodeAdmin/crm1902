...
<script type="text/javascript">
	...
	function showHistory() {
		...
		$.ajax({
			...
			url : "workbench/transaction/getHistroyListByTranId.do",
			data : {
	...
	//改变阶段
	//listStage:需要变更的阶段  i:需要变更的阶段的下标
	function changStage(listStage,i) {
		...
		$.ajax({
			...
			data : {
				...
				"stage" : listStage,
				"id" : "${t.id}",
				"money" : "${t.money}",	//生成交易历史用的信息
				"expectedDate" : "${t.expectedDate}"	//生成交易历史用的信息
			...
			success : function(data){
				...
				if(data.success){
					...
					//阶段改变成功
					//更新页面中的阶段，更新页面中的可能性，更新页面中的修改人，更新页面中的修改时间
	...
	function changeIcon(listStage,index1) {
		...
		//如果当前阶段的可能性为0,前7个阶段的图标一定是黑圈，后两个一个是红叉，一个是黑叉
		...
		//如果当前阶段的可能性不为0，前7个阶段的图标有可能是绿圈，绿色标记，黑圈，后两个一定是黑叉
		}else{
...
<body>
	...
	<div style="position: relative; left: 40px; top: -50px;">
		...
		<%
			...
			//如果当前阶段的可能性为0 前7个图标一定是黑圈 ，后两个一个是红叉，一个是黑叉
			if("0".equals(currentPossibility)){

				//变量每一个阶段
				for(int i=0;i<dvList.size();i++){

					//取得遍历出来的每一个阶段以及每一个阶段对应的可能性
					DicValue dv = dvList.get(i);
			...
			//如果当前阶段的可能性不为0 前7个图标有可能是绿圈，绿色标记，黑圈，后两个一定是黑叉
			}else{
				...
				for(int i=0;i<dvList.size();i++) {
					...
					//如果遍历出来的可能性为0，说明是后两个阶段，后两个一定是黑叉
					if("0".equals(listPossibility)){
					...
					//如果遍历出来的可能性不为0，说明是前7个阶段，前7个图标有可能是绿圈，绿色标记，黑圈
					}else{
